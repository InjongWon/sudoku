...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for r in range(self._n):  # Each row
            for c in range(self._n):  # Each column
                if self._grid[r][c] == EMPTY_CELL and not self._symbol_set - \
                        (self._row_set(r) | self._column_set(c)
                         | self._subsquare_set(r, c)):
                    # Check if this cell is empty and there is no available
                    # symbol for this cell
                    return True
        # Since each cell is filled or has available symbol, it has a solution
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        extensions = self.extensions()  # list of all available solutions
        completed = []  # list of solutions
        while extensions != []:  # until there are no extensions anymore
            curr = []  # for next extensions loop
            for extension in extensions:
                if not extension.is_solved():
                    # If it is not solved, create new extensions
                    curr.extend(extension.extensions())
                elif extension not in completed:
                    # If this new solution is new, add to new extensions
